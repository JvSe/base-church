...
import { getTrackById, updateTrack } from "@/src/lib/actions/tracks";
...
export default function EditTrackPage() {
  ...
  useEffect(() => {
    if (trackData) {
      form.reset({
        ...
        level: (trackData.level as any) || "beginner",
        objectives: trackData.objectives?.join("\n") || "",
        requirements: trackData.requirements?.join("\n") || "",
      });

      if (trackData.image) {
        setBannerPreview(trackData.image);
      }

      setTrackCourses(
        trackData.courses.map((tc: any) => ({
  ...
  const handleMoveCourseUp = (index: number) => {
    ...
    if (prevCourse && currentCourse) {
      [newCourses[index - 1], newCourses[index]] = [currentCourse, prevCourse];
  ...
  const handleMoveCourseDown = (index: number) => {
    ...
    if (currentCourse && nextCourse) {
      [newCourses[index], newCourses[index + 1]] = [nextCourse, currentCourse];
      setTrackCourses(newCourses.map((c, i) => ({ ...c, order: i })));
    }
  };
    ...
    return (
      <PageLayout>
        <LoadingState
          ...
          description="Aguarde enquanto carregamos os dados da trilha"
        />
      </PageLayout>
    );
  }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="dark-text-secondary">
              Atualize as informações e cursos da trilha
            </p>
          </div>
        </div>
      </div>
